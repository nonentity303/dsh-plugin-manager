...
import { readmeIntro } from "./readme-intro.js";
import { npmGlobalRoots } from "./platform.js";
import { aggregateLatest } from "./aggregate.js";
...
/** 源限流熔断：403/429 后冷却 10 分钟（github api 与 dshfind 共享键）。 */
const SOURCE_COOLDOWN = new Map();
const COOLDOWN_MS = 10 * 60 * 1000;

/** 限流冷却键：github/dshfind 共享 api.github.com 配额。 */
function cooldownKey(source) {
	return source?.type === "github" || source?.type === "dshfind" ? "https://api.github.com" : source?.url ?? "";
}

/** 该源是否处于限流冷却期。 */
function isCoolingDown(source) {
	const key = cooldownKey(source);
	if (key === "") return false;
	const until = SOURCE_COOLDOWN.get(key);
	return until !== void 0 && Date.now() < until;
}

/** 标记源限流冷却（403/429 时调用）。 */
function markCooldown(source, status) {
	const key = cooldownKey(source);
	if (key === "") return;
	SOURCE_COOLDOWN.set(key, Date.now() + COOLDOWN_MS);
}

/** 统一返回 {version, repo}：repo 仅供 github/dshfind 源下载时使用。 */
async function fetchLatestVersion(packageName, source) {
	if (isCoolingDown(source)) return { version: null, repo: null };
	if (source.type === "github") return { version: await fetchLatestGithubVersion(packageName, source.url, source), repo: githubRepoOf(source.url) };
	if (source.type === "dshfind") return fetchLatestDshfindVersion(packageName, source);
	...
	try {
		...
		if (response.ok) {
			...
			markCooldown(source, response.status);
		}
	} catch {
		latest = null;
	}
...
async function fetchLatestDshfindVersion(packageName, source) {
	...
	try {
		...
		if (searchResponse.ok) {
			...
			for (const repo of repos) {
				...
				try {
					...
					if (!releaseResponse.ok) {
						if (releaseResponse.status === 403 || releaseResponse.status === 429) markCooldown(source, releaseResponse.status);
						continue;
					}
			...
			markCooldown(source, searchResponse.status);
		}
	} catch {
 ...
 * 失败返回 null（不影响其它源）。
 */
async function fetchLatestGithubVersion(packageName, repoUrl, source) {
	const match = /^https?:\/\/github\.com\/([^/]+)\/([^/]+)/.exec(repoUrl);
...
/** 在所有启用的源中取最高版本及其来源（并行查询；并列时随机，官方源权重一致）。 */
async function fetchLatestAcross(packageName, sources) {
	const enabled = (sources ?? []).filter((source) => source?.enabled === true);
	const results = await Promise.allSettled(
		enabled.map((source) => fetchLatestVersion(packageName, source))
	);
	const entries = results.map((result, index) => {
		const value = result.status === "fulfilled" ? result.value : null;
		return {
			version: value?.version ?? null,
			sourceName: enabled[index].name,
			repo: value?.repo ?? null,
			official: enabled[index].official === true
		};
	});
	return aggregateLatest(entries);
}
...
function resolveBundleExportsPatch(bundle, baseUrl, profileDir) {
	...
	// npm 全局根（跨平台：Windows APPDATA\npm / Linux·macOS npm root -g + 常见路径）
	...
	return class PluginManagerPro extends _classSuper {
		...
		async setSources(sources) {
			return await this.serialize(async () => {
				...
				// 全量重检放后台执行：不阻塞保存操作（并行化后约等于最慢源的一次往返）
				void this.refresh().catch((error) => this.log("refresh", `background refresh failed: ${error instanceof Error ? error.message : String(error)}`));
				return this.snapshot();
			});
		}
		...
		async refresh() {
			...
			const names = [...targets];
			// 8 并发：全量 165 包 × 每包并行查所有源，网络正常时约 20-30s 完成
			for (let i = 0; i < names.length; i += 8) {
				await Promise.all(names.slice(i, i + 8).map((name) => refreshLatest(name, this.sources)));
			}
			return this.snapshot();
		}
		/** 持久化并应用单个条目的期望开关状态。 */
...
`;
		...
		verifyProfile() {
			...
			if (existsSync(patchPath)) {
				// parseDocument 收集错误不抛异常；不能在此用 require("yaml")（host bundle 无 require）
				const doc = parseDocument(readFileSync(patchPath, "utf8"));
				if (doc.errors.length > 0) {
					issues.push({ kind: "patch", name: PATCH_FILENAME, reason: `YAML 解析失败: ${doc.errors.map((e) => e.message).join("; ")}` });
		...
		async fixProfile() {
			return await this.serialize(async () => {
				...
				if (existsSync(patchPath)) {
					...
					if (patchDoc.errors.length > 0 || !Array.isArray(patchDoc.toJS())) {
						patchOk = false;
					}
					if (!patchOk) {
		...
		project(entry) {
			...
			// 防重复挂载识别：同一包已有其它条目启用，或本行 disabled 是 bundle 声明的
			// !!js 动态守卫（如 dsh-remote 与独立 dsh-better-sidebar 双挂载场景）。
			// 这类行被守卫自动停用时，UI 应标注原因并禁止直接启用（启用会覆盖守卫，
			// 下次启动时重复注册 /sidebar/api 等路由导致整树失败）。
			let duplicateOf = null;
			if (!archived) {
				for (const other of this.ctx.loader.entries()) {
					if (other === entry || other.options.group) continue;
					if (packageRoot(other.options.name) !== packageName) continue;
					if (other.disabled) continue;
					duplicateOf = other.options.id;
					break;
				}
			}
			const guarded = isJsExpr(entry.options?.disabled);
			const mountConflictReason = !enabled && (guarded || duplicateOf !== null)
				? guarded
					? `防重复挂载守卫自动停用：${duplicateOf ?? "其它条目"} 正在提供 ${packageName}。启用本行会覆盖守卫，下次启动时因重复路由（如 /sidebar/api）失败；如需切换请先停用 ${duplicateOf ?? "提供方"}。`
					: `${duplicateOf} 已启用同一包（${packageName}），启用本行会造成重复挂载并在下次启动时失败；如需切换请先停用 ${duplicateOf}。`
				: null;
			const info = packageInfoOf(entry.options.name, this.baseUrl);
			...
			// 简介：内置目录优先，其次从包 README（zh 优先）/ package.json.description 提取
			let description = DESCRIPTIONS[configId];
			if (description === void 0) {
				description = readmeIntro(packageName, this.readmeRoots()) ?? "（无简介）";
			}
			return {
				entryId: entry.id,
				configId,
				...
				duplicateOf,
				mountConflictReason,
				origin: spec !== void 0 ? "user" : "builtin",
				installedVersion: installed,
		...
		/** 简介提取的候选 node_modules 根：profile 本地 > profiles 共享 > 全局 npm。 */
		readmeRoots() {
			const roots = [join(this.location.directory, "node_modules")];
			try {
				roots.push(join(resolveDshHome(), "profiles", "node_modules"));
			} catch { /* ignore */ }
			for (const root of npmGlobalRoots()) roots.push(root);
			return roots;
		}
		async change(entry, enabled) {
			...
			// 防重复挂载保护：目标行当前被守卫停用且同包已有启用挂载（或本行带
			// !!js 动态守卫）时，拒绝写入 enabled=true —— 覆盖守卫会让同一包在
			// 下次启动时双挂载，重复注册 /sidebar/api 等路由导致整树启动失败。
			if (enabled === true && entry.mountConflictReason !== null) return {
				entryId: entry.entryId,
				status: "skipped",
				message: entry.mountConflictReason
			};
		...
		async waitFor(entryId, enabled) {
			// 等待插件状态同步：当当前状态不匹配目标状态且无初始化/清理任务时继续等待
			// undefined 视为未禁用（启用中），false=已启用，true=已禁用
			const deadline = Date.now() + this.settleTimeoutMs;
			while (Date.now() < deadline) {
				const entry = this.ctx.loader.resolve(entryId);
				if ((entry.disabled ?? false) !== enabled && entry._initTask === void 0 && entry._disposing === 0) return;
		...
		log(action, detail) {
			const message = `[plugin-manager] ${action}${detail !== void 0 ? `: ${detail}` : ""}`;
			...
			try {
				...
				// 宿主logger失败，尝试控制台输出
				try {
					console.info(`[plugin-manager-fallback] ${action}${detail !== void 0 ? `: ${detail}` : ""}`);
				} catch (consoleError) {
					// 控制台输出也失败，写入临时日志文件
					try {
						const logDir = tmpdir();
						const logFile = join(logDir, 'dsh-plugin-manager.log');
						const timestamp = new Date().toISOString();
						const logEntry = `[${timestamp}] INFO ${message}\n`;
						
						// 简单的追加写入，不依赖复杂的文件系统操作
						appendFileSync(logFile, logEntry, 'utf8');
					} catch (fileError) {
						// 所有日志记录方式都失败，至少保留错误信息
						console.error(`[plugin-manager] 所有日志记录方式均失败:`, loggerError?.message || consoleError?.message || fileError?.message);
					}
				}
			}
		}
...
/**
 * 判断 Loader 条目的 disabled 是否为 !!js 动态守卫表达式节点。
 * cordis-plugin-loader 对 !!js disabled 会在每次读取时实时求值（entry.disabled
 * getter），管理器需要识别这类行，避免用补丁覆盖守卫造成同包双挂载。
 */
...
/* Config 必须是 zod Standard Schema（.default({})），否则 cordis loader 启动即崩。
 * 见 dev-docs/DSH插件开发指南.md §2.2。 */
export const Config = z.object({
	/** 额外受保护条目 id 列表（不会被管理器开关/隔离）。 */
	protectedEntries: z.array(z.string()).default([]),
	/** 开关状态变更后等待收敛的超时（毫秒）。 */
	settleTimeoutMs: z.number().min(100).max(120000).default(8000)
}).default({});
